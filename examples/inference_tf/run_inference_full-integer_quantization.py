...
def run_inference(image_path, model_path, labels_path):
    ...
    # Obtener las 5 clases con mayor puntaje (logit)
    top_5_indices = np.argsort(output_float)[-5:][::-1]
    print("\nResultados por puntaje (logit):")
    ...
    top_5_indices_prob = np.argsort(probabilidades)[-5:][::-1]
    print("\nResultados por probabilidad:")
    for i in top_5_indices_prob:
        print(f"Clase {labels[i]} ({i}) con una probabilidad de {probabilidades[i] * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    model_path = 'results/mobilenet_trap-colour-insects/mobilenet_full_integer_quant.tflite'  # Ruta del modelo TFLite
